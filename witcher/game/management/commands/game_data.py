...
''',
        'actions': {' Взять трофей. Отправиться в деревню за наградой.': (26, None)}
    },
    {
        'name': '7',
...
''',
        'actions': {'-"Спасибо"[взять награду]': (16, None),
                    'Я не могу взять. Тролль все ещё жив." [отправиться к троллю]': (28, None)}
    },
    {
        'name': '12',
...
выл староста. 
''',
        'actions': {'Вывести старосту на улицу и рассказать о том, что староста пользовался спившимся троллем, а все убийства и грабежи прохожих лежат насовести Бориса]( Геральт останется без денег) ': (10, None),
                    'Обезглавить Бориса': (24, None),
                    '-"Хмм, тролль мёртв". Спокойно забрать деньги': (16, None)}
    },
    {
        'name': '16',
...
''',
        'actions': {'Обвинить Бориса в том, что он изначально нанимал людей, чтобы убить двух троллей, но выжил только один. Староста решил намеренно споить того, чтобы получать с него деньги за разграбление прохожих.': (7, None)}
    },
    {
        'name': '21',
        'message': '''''',
        'actions': {'Рассказать троллю о записке': (3, None),
...
''',
        'actions': {'-"Что за беда у вас с троллем?"': (18, None),
                    '-"Впервые вижу в Новой Темерии, чтобы был такой контраст между домом старосты и остальными халупами. Вижу, хорошо вы устроились': (13, None)}
    },
    {
        'name': '23',
...
''',
        'actions': {'Обвинить Бориса в том, что он изначально нанимал людей, чтобы убить двух троллей, но выжил только один. Староста решил намеренно споить того, чтобы получать с него деньги за разграбление прохожих': (7, None),
                    ...
                    'Шантажировать старосту тем фактом, что знаешь о намеренном спаивании тролля с целью грабежа прохожих людей. Рассказать деревне о том, что все бедства деревни связаны с этим': (7, None)}
    },
    {
        'name': '28',
        'message': '''-Угу.
...
''',
        'actions': {'Получается, Борис изначально нанимал кого-то, чтобы убить троллей. Видно, не очень у них это получилось. Но зачем староста стал платить троллю за грабёж прохожих, но говорит, что тот должен был достроить мост? Ерунда какая-то': (20, None)}
    },
    {
        'name': '29',
...
ТОЙ СКОТИНА?!"
''',
        'actions': {'Да, твоя жена отомщена.': (11, None)}
    },
    {
        'name': '30',
...
''',
        'actions': {'Взять трофей и отправиться в деревню.': (26, None)}
    },
    {
        'name': '33',
...
старосты Бориса...
''',
        'actions': {'Взять трофей и отправиться в деревню за наградой.': (26, None)}
    },
    {
        'name': '35',
